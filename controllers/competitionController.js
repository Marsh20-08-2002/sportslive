...
// Controller to fetch 3 random competitions
exports.getRandomCompetitions = catchAsync(async (req, res, next) => {
  const randomCompetitions = await Org.aggregate([
    { $sample: { size: 3 } }, // Fetch 3 random documents
  ]);

  if (!randomCompetitions.length) {
    return next(new AppError("No competitions found", 404));
  }

  res.status(200).json({
    status: "success",
    results: randomCompetitions.length,
    data: {
      data: randomCompetitions,
    },
  });
});
